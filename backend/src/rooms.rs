...
pub async fn get_revealed_tiles(client: &RedisClient, room_id: &str) -> Result<u32, RedisError> {
    client
...
pub async fn room_exists(client: &RedisClient, room_id: &str) -> bool {
    client.exists(room_id).await.unwrap()
}

pub async fn create_room(client: &RedisClient, custom_room_id: Option<Uuid>) -> Uuid {
    let room_id = custom_room_id.unwrap_or_else(|| Uuid::new_v4());
